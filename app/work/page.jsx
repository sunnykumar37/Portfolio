...
const WorksContent = () => {
  ...
  return (
    ...
    className="min-h-[80vh] flex flex-col justify-center py-10 sm:py-12 xl:px-0"
    >
     <div className="container mx-auto">
      
    <div className="flex flex-col gap-10 xl:flex-row xl:gap-[30px]">
      ...
      <div className="w-full xl:h-[460px] flex flex-col xl:justify-between order-2 xl:order-none">
        <div className="flex flex-col gap-6 sm:gap-[30px]">
          ...
          <div>
            <h2 className="text-[32px] sm:text-[42px] font-bold leading-none text-white hover:text-accent transition-all duration-500 capitalize">{project.category} project</h2>
            <p className="text-sm sm:text-base text-white/60">{project.description}</p>
            <ul className="flex flex-wrap gap-2 sm:gap-3">
              {
                project.stack.map((item, index) =>{
                  return <li key={index} className="text-base sm:text-xl text-accent">
                        {item.name}
            ...
            <div className="py-2">
              <Link href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 sm:gap-5">
              <h3 className="text-base sm:text-xl text-white/50 break-words">Check it out!!...</h3>
              <div className="h-9 w-9 sm:h-10 sm:w-10 hover:bg-white rounded-full border items-center flex justify-center shrink-0">
              <BsArrowUpRight className="text-accent text-3xl hover:text-black   transition-transform duration-300 hover:rotate-45" />
              </div>
              </Link>

              <Link href={project.repo} className="flex items-center gap-3 sm:gap-5">
              <h3 className="text-base sm:text-xl text-white/50 break-words">Check the repository!!...</h3>
              <div className="h-9 w-9 sm:h-10 sm:w-10 hover:bg-white rounded-full border items-center flex justify-center shrink-0">
      ...
      <div className="w-full xl:w-[50%]">
        <div className="relative">
          ...
          spaceBetween={30}
          slidesPerView={1}
          className="h-[310px] sm:h-[420px] xl:h-[520px]"
          onSlideChange = {handelSlideChange}
          onSwiper={setSwiperRef}
          >
            {projects.map((item, index) =>{
              return (<SwiperSlide key={index} className="w-full">
                <div className="h-[280px] sm:h-[360px] xl:h-[460px] relative group flex justify-center items-center bg-pink-50/20">
                <div></div>
                  <div className="relative w-full h-full">

                    <Image src={item.image} fill  alt={item.title} className="object-contain p-2 sm:p-4"/>
          ...
          <button
            ...
            className="group absolute left-2 sm:left-3 top-1/2 z-20 -translate-y-1/2 h-9 w-9 sm:h-11 sm:w-11 rounded-full border border-white/30 bg-primary/70 text-white backdrop-blur-sm flex items-center justify-center transition-all duration-300 hover:bg-accent hover:border-accent hover:text-primary hover:scale-105 disabled:opacity-35 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
          ...
          <button
            ...
            className="group absolute right-2 sm:right-3 top-1/2 z-20 -translate-y-1/2 h-9 w-9 sm:h-11 sm:w-11 rounded-full border border-white/30 bg-primary/70 text-white backdrop-blur-sm flex items-center justify-center transition-all duration-300 hover:bg-accent hover:border-accent hover:text-primary hover:scale-105 disabled:opacity-35 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
